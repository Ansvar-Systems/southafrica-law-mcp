/**
 * validate_eu_compliance — Check SA statute's EU/international alignment status.
 *
 * South Africa is not an EU member, so this tracks "alignment" rather than
...
export interface EUComplianceResult {
  document_id: string;
  provision_ref?: string;
  compliance_status: 'aligned' | 'partial' | 'unclear' | 'not_applicable';
...
export async function validateEUCompliance(
  ...
  if (rows.length === 0) {
    recommendations.push(
      'No international references found. South Africa may have domestic-only legislation for this area.'
    );
  } else if (primaryCount === 0) {
    warnings.push('International references exist, but none are marked as primary alignment.');
    recommendations.push('Review reference quality and verify alignment status with the source instrument.');
  }

  warnings.push(
    'South Africa is not an EU member state. Alignment status reflects influence, not legal obligation.'
  );

  const status: EUComplianceResult['compliance_status'] =
    rows.length === 0 ? 'not_applicable' :
    primaryCount > 0 ? 'aligned' :
    'partial';
