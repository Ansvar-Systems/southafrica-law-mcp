/**
 * search_eu_implementations — Search EU/international instruments with SA implementation info.
 */
...
export interface SearchEUImplementationsInput {
  query?: string;
  type?: 'directive' | 'regulation' | 'convention';
  year_from?: number;
  year_to?: number;
  community?: string;
  has_sa_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      COUNT(DISTINCT er.document_id) AS sa_statute_count,
  ...
  if (input.has_sa_implementation !== undefined) {
    sql += input.has_sa_implementation ? ` HAVING sa_statute_count > 0` : ` HAVING sa_statute_count = 0`;
  }
  ...
  interface Row {
    ...
    sa_statute_count: number; primary_implementations: string | null;
  }
  ...
  return {
    results: {
      results: rows.map(r => ({
        ...
        sa_statute_count: r.sa_statute_count,
