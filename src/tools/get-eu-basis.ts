/**
 * get_eu_basis — Retrieve EU/international legal basis for a South African statute.
 *
 * South Africa is not an EU member, but several SA laws were influenced by
 * EU instruments (GDPR -> POPIA) and international conventions (Budapest Convention -> Cybercrimes Act).
 */
...
export async function getEUBasis(
  ...
  interface QueryRow {
    id: string; type: 'directive' | 'regulation' | 'convention'; year: number; number: number;
  ...
  return {
    results: {
      ...
      statistics: {
        total_references: euDocuments.length,
        ...
        convention_count: euDocuments.filter(d => d.type === 'convention').length,
      },
    },
