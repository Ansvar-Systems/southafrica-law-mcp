#!/usr/bin/env tsx
/**
 * Database builder for South Africa Law MCP server.
 *
...
type EUDocumentType = 'directive' | 'regulation' | 'convention';
type EUCommunity = 'EU' | 'EC' | 'CoE' | 'AU' | 'SADC';
type EUReferenceType = 'implements' | 'influenced_by' | 'aligned_with' | 'references';
...
const SCHEMA = `
-- Legal documents (statutes, constitution)
CREATE TABLE legal_documents (
  id TEXT PRIMARY KEY,
  type TEXT NOT NULL CHECK(type IN ('statute', 'constitution', 'regulation', 'bill', 'case_law')),
  title TEXT NOT NULL,
  title_en TEXT,
  short_name TEXT,
...
CREATE TABLE eu_documents (
  id TEXT PRIMARY KEY,
  type TEXT NOT NULL CHECK (type IN ('directive', 'regulation', 'convention')),
  year INTEGER NOT NULL CHECK (year >= 1900 AND year <= 2100),
  number INTEGER NOT NULL CHECK (number >= 0),
  community TEXT CHECK (community IN ('EU', 'EC', 'EEC', 'Euratom', 'CoE', 'AU', 'SADC')),
  celex_number TEXT,
  title TEXT,
  title_en TEXT,
  short_name TEXT,
...
-- EU/International References (links SA provisions to international documents)
CREATE TABLE eu_references (
  ...
  reference_type TEXT NOT NULL CHECK (reference_type IN (
    'implements', 'supplements', 'applies', 'references', 'influenced_by',
    'aligned_with', 'cites_article'
  )),
...
function extractReferences(text: string): ExtractedReference[] {
  ...
  // EU Regulation/Directive patterns
  const euPatterns: RegExp[] = [
    /\b(Regulation|Directive)\s*\((EU|EC|EEC)\)\s*(?:No\.?\s*)?(\d{2,4})\/(\d{1,4})\b/gi,
    /\b(Regulation|Directive)\s*(?:No\.?\s*)?(\d{2,4})\/(\d{1,4})\/(EU|EC|EEC)\b/gi,
  ...
  for (const pattern of euPatterns) {
    ...
    while ((match = pattern.exec(text)) !== null) {
      ...
      if (pattern === euPatterns[0]) {
        ...
        rawYear = match[2]; rawNumber = match[3];
      }

      const year = normalizeYear(rawYear);
      const number = parseInt(rawNumber, 10);
      if (year <= 0 || isNaN(number) || number <= 0) continue;

      const community = (communityRaw?.toUpperCase() as EUCommunity) ?? 'EU';
      ...
      refs.push({
        ...
        fullCitation: match[0], referenceContext,
        referenceType: inferReferenceType(referenceContext),
      });
    }
  }
...
function normalizeYear(rawYear: string): number {
  ...
  if (rawYear.length === 2) return parsed >= 50 ? 1900 + parsed : 2000 + parsed;
  return parsed;
}

function inferReferenceType(context: string): EUReferenceType {
  if (/\b(implement|transpos|gives effect)\b/i.test(context)) return 'implements';
  if (/\b(influenced|modeled|inspired|based on)\b/i.test(context)) return 'influenced_by';
  if (/\b(aligned|align|conform)\b/i.test(context)) return 'aligned_with';
  return 'references';
}
...
function buildDatabase(): void {
  console.log('Building South Africa Law MCP database...\n');
  ...
  const insertDoc = db.prepare(`
    INSERT INTO legal_documents (id, type, title, title_en, short_name, status, issued_date, in_force_date, url, description, language)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const loadAll = db.transaction(() => {
    for (const file of seedFiles) {
      ...
      for (const prov of deduped) {
        ...
        const extractedRefs = extractReferences(prov.content);
        if (extractedRefs.length > 0) {
          const sourceId = `${seed.id}:${prov.provision_ref}`;
          const lastVerified = new Date().toISOString();

          for (const ref of extractedRefs) {
            const eurLexType = ref.type === 'regulation' ? 'reg' : ref.type === 'directive' ? 'dir' : '';
            const eurLexUrl = eurLexType
              ? `https://eur-lex.europa.eu/eli/${eurLexType}/${ref.year}/${ref.number}/oj`
              : null;
            const shortName = ref.type === 'convention'
              ? ref.fullCitation
            ...
            const euInsert = insertEuDocument.run(
              ...
              'Auto-extracted from South African statute text',
            );
            if (euInsert.changes > 0) totalEuDocuments++;

            const primaryKey = `${seed.id}:${ref.euDocumentId}`;
            const isPrimary =
              (ref.referenceType === 'implements' || ref.referenceType === 'influenced_by')
              && !primaryImplementationByDocument.has(primaryKey) ? 1 : 0;
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'ZA');
    insertMeta.run('source', 'saflii.org');
    insertMeta.run('licence', 'Public Domain (South African legislation)');
  });
  writeMeta();
  ...
  console.log(
    ...
    `${totalDefs} definitions, ${totalEuDocuments} EU/intl documents, ${totalEuReferences} references`
  );
  if (emptyDocs > 0) {
